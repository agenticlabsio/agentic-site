...
- **No ESLint**: The project has no ESLint configuration or `lint` script. TypeScript checking (`npx tsc --noEmit`) can be used instead; note there are pre-existing type errors in `src/seed/` and `src/app/(payload)/api/_seed/` files.
...
- `npx tsc --noEmit` — type checking (in lieu of lint)
